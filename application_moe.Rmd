...
# Pull out the sites from previous years that we will be returning to again. We will grab these sites from the bcfishpass database
ids <- c(
  #arctic
  125000, 
  #fern
...
# Now pull out all the new sites that we have identified in the planning file. 
...
sites_for_review <- fpr::fpr_db_query(
  glue::glue(
    "SELECT * FROM bcfishpass.crossings_vw 
     WHERE stream_crossing_id IN ({glue::glue_collapse(ids, sep = ', ')})"
  )
)
