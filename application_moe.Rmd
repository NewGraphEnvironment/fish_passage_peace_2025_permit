...
params:
  repo_url: 'https://github.com/NewGraphEnvironment/fish_passage_peace_2025_permit'
  report_url: 'https://NewGraphEnvironment.github.io/fish_passage_peace_2025_permit/'
...
link_kml <- fs::path(params$repo_url, '/raw/main/docs/', paste0("sites_", project_region, "_", permit_year, "_", format(Sys.Date(), '%Y%m%d'), "_kml.zip"))

# Specific the target species likely to be tagged since we include so many. Varies from region to region and can be determined from
...
`r if (gitbook_on == FALSE) knitr::asis_output(paste0("This permit application can also be viewed online [at this link](", params$report_url, ")."))` A summary of sites proposed for assessment, including historic fish presence records from FISS, is provided in Tables \@ref(tab:tab-sites1) to \@ref(tab:tab-sites3). Fish species known to occur within each watershed are summarized in Table \@ref(tab:tab-fish). An overview map showing potential sample locations is presented in Figure \@ref(fig:map). A KML and GPX file of all sites is attached to the application and can also be downloaded [at this link](`r link_kml`) (or [here](`r paste0(params$repo_url,"/tree/main/mapping")`) if this is not the latest version of the memo). The KML includes detailed site-specific information accessible by clicking on each location, with brief summaries of background reports where available.
...
- https://newgraphenvironment.github.io/fish_passage_peace_2024_reporting/
...
site_review_ids <- c(
  ...
  #no outlet drop, ok flow and decent looking habitat. On reserve
  # 198667
  )
...
sites_planning <- sites_planning_raw |> 
  dplyr::filter(
    my_priority %in% c("mod", "high") &
      watershed_group_code %in% c("NATR", "PARA")
  )
...
table_sites <- dplyr::left_join(
  ...
  # add the 3 missing watershed codes for the crossings that didn't have `modelled_crossing_id`
  dplyr::mutate(wsc_code = dplyr::case_when(
    id == 125000 ~ "236-738000-00000-00000-0000-0000-000-000-000-000-000-000",
    id == 7819 ~ "230-870800-11500-24500-0000-0000-000-000-000-000-000-000",
    id == 7827 ~ "230-870800-00000-00000-0000-0000-000-000-000-000-000-000",
    T ~ wsc_code)
  ) |> 
...
formatted_output <- table_sites |> 
  ...
  dplyr::select(-max_id_width, -max_name_width) %>%  # Drop the max_* columns
  {
    ...
    formatted_rows <- purrr::pmap_chr(., ~ paste(..., collapse = "\t"))
...
# Zip up all gpx and KMLs (should just be today's file)
...
fs::file_delete(
  fs::dir_ls("docs/", glob = "*.zip")
)

zip::zipr(
  zipfile = paste0("docs/", prefix, today_date, "_kml.zip"),
  files = files_to_zip
)
...
mybasemap <- ggmap::get_map(location = c(left = table_sites |> dplyr::pull(long) |> min()-aoi_buffer, 
                                    bottom = table_sites |> dplyr::pull(lat) |> min()-aoi_buffer,
                                    right = table_sites |> dplyr::pull(long) |> max()+aoi_buffer,
                                    top = table_sites |> dplyr::pull(lat) |> max()+aoi_buffer),
                     source = "google",
                     zoom = 7,
                    maptype = "roadmap")
...
# mybasemap <- ggmap::get_map(location = c(lon = table_sites |> dplyr::pull(long) |> mean(),
#                                          lat = table_sites |> dplyr::pull(lat) |> mean())
#                             source = "google",
#                             zoom = "auto",
#                             maptype = "hybrid")
...
tab_fish <- readr::read_csv('data/fiss_species_table.csv') |> 
  dplyr::select(-`Peace Arm`, -`Upper Peace`)
